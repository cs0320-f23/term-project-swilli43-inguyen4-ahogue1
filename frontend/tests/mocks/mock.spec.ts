...
test("pressing the next and previous buttons provides expected functionality", async ({
  ...
  await expect(page.getByText(initPrompt)).toBeVisible();

  const initDate = "Mock: Dec 18, 2023";
  await expect(page.getByText(initDate)).toBeVisible();
  ...
  await expect(page.getByText(prevPrompt)).toBeVisible();

  const prevDate = "Mock: Dec 11, 2023";
  await expect(page.getByText(prevDate)).toBeVisible();

  const prevEntry = "Mock: I've been really sad, but also really grateful.";
  await expect(page.getByText(prevEntry)).toBeVisible();
