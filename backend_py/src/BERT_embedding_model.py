...
def generate_personalized_suggestions(user_entry: str):
    ...
    else: # query the LLM again with an updated propmt 
        raw_suggestions2 = reformat_LLM_suggestions(raw_suggestions)
        if valid_format(raw_suggestions2):
            return raw_suggestions2
        
        # If the suggestions are still incorrectly formatted after a second call to the LLM, 
        # instead, return 10 randomly selected suggestions from a pre-determined bank of wellbeing
        # suggestions. 
        # For system performance sake, we decided to cap the # of calls to an LLM to 2 attempts,
        # so that the system wouldn't delay too long if the LLM struggles to generate a properly
